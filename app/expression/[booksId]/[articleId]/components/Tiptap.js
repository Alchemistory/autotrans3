...
import { Input, Button, Popover, PopoverTrigger, PopoverContent,Spinner } from "@nextui-org/react";
import { useState,useEffect } from "react";
import {createClient} from "@/utils/supabase/client"
import { ToastContainer, toast } from "react-toastify";

function SimpleTiptap({booksId,articleId,expression,expressionList,filteredExpressionList,getExpression,getExpressionList,changeFlag,setChangeFlag,activePopover,setActivePopover}) {
  ...
  const highlightText = (text, words, expressionList) => {
    ...
    if (words.length) {
      ...
      highlightedText = highlightedText.replace(
        wordsRegex,
        (match) => `<span class="selected-word" style="background-color: yellow; cursor: pointer;">${match}</span>`
      );
    }
...
<div className="w-full h-full flex flex-col gap-y-2">
      ...
      {activePopover === expression.chunkId.id && selectedText && (
        <Popover isOpen={activePopover === expression.chunkId.id} placement="top" offset={5}>
          <PopoverTrigger>
            ...
            <div className="text-medium py-2 px-4 relative flex flex-col items-center">
              ...
              <div className="flex flex-col gap-y-2 mt-2">
                {selectionType === 'drag' ? (
                  ...
                  <div className="flex flex-row gap-x-2">
                    <Button size='sm' color='primary' variant='flat'>수정</Button>
                    <Button size='sm' color='primary' variant='flat'>삭제</Button>
                  </div>
                )}
              </div>
            </div>
